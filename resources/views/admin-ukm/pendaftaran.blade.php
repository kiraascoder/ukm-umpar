...
@section('content')
    <div x-data="{ openModal: null, selectedNama: '', deleteUrl: '' }" class="bg-gray-100 min-h-screen p-6">
        <h1 class="text-2xl text-gray-800 mb-6">Manajemen Pendaftaran UKM</h1>

        <div class="lg:col-span-3 w-full bg-white p-6 rounded-2xl shadow overflow-auto">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-gray-700">Daftar Pendaftaran UKM</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($pendaftaran as $item)
                    <div class="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300">
                        ...
                        <div class="p-6">
                            <h3 class="text-xl font-bold mb-2">{{ $item->pendaftaran }}</h3>
                            <p class="text-gray-600 mb-4">
                                Batas Pendaftaran : {{ Str::limit($item->batas_pendaftaran, 100, '...') }}
                            </p>
                            <div class="flex justify-between">
                                <a href="{{ route('adminUkmDetailPendaftaran', ['id' => $item->id]) }}"
                                    class="text-[#608BC1] font-medium hover:text-yellow-600 transition inline-flex items-center">
                                    <span>Lihat Detail</span>
                                    <i class="fas fa-arrow-right ml-2 text-sm"></i>
                                </a>
                                <button type="button"
                                    @click="openModal = {{ $item->id }}; selectedNama = '{{ $item->nama }}'; deleteUrl = '{{ route('adminUkmPendaftaran.delete', $item->id) }}'"
        ...
        <div x-cloak x-show="openModal" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="bg-white p-6 rounded-xl shadow-xl w-96">
                ...
                <p class="text-sm text-gray-600 mb-6">
                    Yakin ingin menghapus Pendaftaran <strong x-text="selectedNama"></strong>?
                </p>
